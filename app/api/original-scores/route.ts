...
export async function POST(request: Request) {
  ...
  // Update original_personal_bests
  const { data: existing } = await supabase
    .from('original_personal_bests')
    .select('*')
  ...
  // Update original_daily_scores
  const { data: existingDaily } = await supabase
    .from('original_daily_scores')
    .select('*')
    .eq('player_id', player_id)
    .eq('date', today)
    .single()
  ...
  // Clean up old daily scores
  const twoDaysAgo = new Date(Date.now() - 48 * 60 * 60 * 1000 + 12 * 60 * 60 * 1000).toISOString().split('T')[0]
  await supabase
    .from('original_daily_scores')
    .delete()
    .lt('date', twoDaysAgo)
