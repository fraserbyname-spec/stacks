...
type Score = { id: number; player_name: string; balance: number; picks: number }
type PlayerRank = { rank: number; player_name: string; balance: number; picks: number }

export default function OriginalLeaderboard() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/original-scores?player_id=${player_id}`)
      .then(r => r.json())
    ...
    <main className="min-h-screen bg-[#F4F6F8] flex items-center justify-center p-4">
      <div className="bg-white rounded-3xl shadow-sm w-full max-w-sm p-8 flex flex-col gap-6">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-[#1A2B3C]">World&apos;s Biggest Stackers</h1>
          <p className="text-[#7F8C8D] text-sm mt-1">Original game</p>
        </div>
        {loading ? <p className="text-center text-[#7F8C8D] text-sm">Loading...</p> :
          top10.length === 0 ? <p className="text-center text-[#7F8C8D] text-sm">No scores yet. Be the first!</p> : (
